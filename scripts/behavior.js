...
function link(message, username){
    ifLinked(message.author.id, function(isLinked){
        if (!isLinked){
            linker.start();
            sendMessage(message.author, "You have requested to link account with faf account `"+username+"`. To proceed, please open the address "
				+ linker.getAddress(message.author.id)
				+ " in your browser and log-in.\nYou have **30 SECONDS** before the link expires.");

            linker.status.on("success", function(login, id, author_id){
                if (login === username && author_id === message.author.id){
